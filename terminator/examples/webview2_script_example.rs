            ...
            if let Ok(Some(url)) = element.url().await {
                if !url.is_empty() {
                    ...
                    for (description, script) in test_scripts {
                        ...
                        match element.execute_script(script).await {
                            Ok(Some(result)) => {
                                info!(
                                    "✅ {} - Result: {}",
                                    description,
                    ...
                    break; // Found one WebView2 control, that's enough for the example
                }
            }
        }
    }
