use terminator::{Desktop, AutomationError, TextPosition, FontStyle, HighlightHandle};
...
async fn main() -> Result<(), AutomationError> {
    ...
    loop {
        ...
        if let Some(calculator) = apps.iter()
                        ...
                        Ok(buttons) => {
                            ...
                            let element_changed = match (&last_highlighted_element, &found_element) {
                                ...
                                (Some(last), Some(current)) => {
                                    ...
                                    last.name().unwrap_or_default() != current.name().unwrap_or_default()
                                }
                            };
                            ...
                            if element_changed {
                                ...
                                if let Some(ref element) = found_element {
                                    let element_name = element.name().unwrap_or("Element".to_string());
