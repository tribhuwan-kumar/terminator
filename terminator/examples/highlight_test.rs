use terminator::{Desktop, AutomationError, TextPosition, FontStyle};
...
async fn main() -> Result<(), AutomationError> {
    ...
    let calculator = apps.iter()
        .find(|app| app.name().unwrap_or_default().to_lowercase().contains("calculator"))
        .ok_or_else(|| AutomationError::PlatformError("Calculator not found. Please open Calculator first.".to_string()))?;
    
    println!("✅ Found Calculator: {}", calculator.name().unwrap_or("Unknown".to_string()));
    ...
    let button = match calculator.locator("role:Button") {
        Ok(locator) => match locator.first(None).await {
            Ok(button) => {
                println!("✅ Found a button: {}", button.name().unwrap_or("Unknown".to_string()));
                button
            }
            Err(e) => {
                ...
                match calculator.locator("role:*") {
                    Ok(locator) => match locator.first(None).await {
                        Ok(element) => {
                            println!("✅ Found a clickable element: {} ({})", 
