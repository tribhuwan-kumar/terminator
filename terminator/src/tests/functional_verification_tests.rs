...
fn test_get_application_by_name_functional() {
    ...
    assert!(found_app, "Should be able to find at least one system application");
        ...
        Ok(apps) => {
            if let Some(app) = apps.first() {
                ...
                for i in 1..=3 {
                    ...
                    println!("      ✅ Properties map accessible with {} entries", attrs.properties.len());
                }
                
                // Test that direct role access still works
                let direct_role = app.role();
                let attrs_role = app.attributes().role;
                println!("    ✅ Direct role: '{}', Attrs role: '{}'", direct_role, attrs_role);
                
                // They should be the same
                assert_eq!(direct_role, attrs_role, "Direct role and attributes role should match");
            }
        }
        Err(e) => {
                        ...
                        Ok(app2) => {
                            ...
                            if let Ok(pid2) = app2.process_id() {
                                ...
                                if second_duration < first_duration {
                                    println!("    ✅ Cache optimization detected (faster second lookup)");
                                } else {
                                    println!("    ⚠️  Second lookup not faster, but that's still okay");
                                }
                            }
                        }
...
fn test_error_handling_robustness() {
    ...
    let invalid_names = ["", "   ", "invalid/name", "app\0with\0nulls", "very_long_name_that_definitely_does_not_exist_in_any_system_12345"];
