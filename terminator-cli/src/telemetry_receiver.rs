...
async fn process_json_traces(
    data: serde_json::Value,
    steps: Arc<Mutex<StepsTracker>>,
) {
...
async fn process_span(span: &serde_json::Value, tracker: &Arc<Mutex<StepsTracker>>) {
                ...
                match event_name {
                    ...
                    "step.started" => {
                        if let Some(tool) = event_attrs.get("step.tool") {
                            let step_index = event_attrs.get("step.index")
                    ...
                    "workflow.completed" => {
                        let had_errors = event_attrs.get("workflow.had_errors")
    ...
    if name.starts_with("workflow.") {
        ...
        if let Some(tool) = attributes.get("tool.name") {
            let step_num = attributes.get("step.number")
                .and_then(|s| s.parse::<usize>().ok())
                .unwrap_or(0);
            let step_total = attributes.get("step.total")
                .and_then(|s| s.parse::<usize>().ok())
                .unwrap_or(0);
            ...
            for span in scope_span.spans {
                ...
                if span_name.starts_with("step.") {
                    ...
                    if let Some(tool) = span_attrs.get("tool.name") {
                        let step_num = span_attrs.get("step.number")
                            .and_then(|s| s.parse::<usize>().ok())
                            .unwrap_or(0);
                        let step_total = span_attrs.get("step.total")
                            .and_then(|s| s.parse::<usize>().ok())
                            .unwrap_or(0);
...
fn extract_proto_attr_value(value: &Option<opentelemetry_proto::tonic::common::v1::AnyValue>) -> String {
    if let Some(val) = value {
        if let Some(v) = &val.value {
            match v {
                ...
                opentelemetry_proto::tonic::common::v1::any_value::Value::DoubleValue(f) => f.to_string(),
