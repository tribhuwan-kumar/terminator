...
# Type some text
await editor.click()
await editor.type_text("Hello from Python automation!\n")
await sleep(500)
await editor.type_text("This is an automated test using Terminator MCP.\n")
await sleep(500)
await editor.type_text("\nDemonstrating keyboard shortcuts:\n")
await sleep(500)

# Select all text (Ctrl+A)
log("Selecting all text...")
await editor.press_key("{Control+a}")
await sleep(1000)

# Deselect and move to end
await editor.press_key("{End}")
await sleep(500)

# Type more text
await editor.type_text("\nAutomation complete!\n")
await editor.type_text(f"Timestamp: {__import__('datetime').datetime.now().isoformat()}")
...
if file_menu:
    ...
    await sleep(1000)
    # Close menu
    await desktop.press_key("{Escape}")
...
try:
    text_content = await editor.value()
