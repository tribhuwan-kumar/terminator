...
class NetronlineHighlightTest {
  ...
  async run() {
    ...
    // Allow page to render footer
    await delay(1500);

    // Step 2: Highlight footer link (requires scrolling)
    const targetText = 'Do Not Sell or Share My Personal Information';
    // Inspector shows ControlType: Button, so prefer Button role
    const selector = `role:button|name:${targetText}`;
    console.log(`\n🎯 Highlighting selector: ${selector}`);
    const highlightContent = await this.callTool('highlight_element', {
      selector,
      // API expects a single string; provide comma-separated alternatives
      alternative_selectors: `role:button|name:${targetText},role:hyperlink|name:${targetText}`,
