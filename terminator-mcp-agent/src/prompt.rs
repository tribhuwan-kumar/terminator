...
*   Most action tools default `include_tree` to `false` to keep responses fast. When you need the UI tree included in a tool result, pass `include_tree: true` explicitly. For tree extraction tools, you can optimize with `include_tree: { max_depth: 2, from_selector: "role:Button" }` to limit depth or get subtrees.
...
2.  **Get the UI Tree:** This is the most important step. Once you have the `pid` of your target application, call `get_window_tree` to retrieve the current UI tree. Use `include_detailed_attributes` to control attribute depth (defaults to true). For performance optimization:
    ...
    - Use `include_tree: { from_selector: "role:Dialog" }` to get subtree starting from a specific element
    - Use `include_tree: { from_selector: "true" }` with `get_focused_window_tree` to start from the focused element
