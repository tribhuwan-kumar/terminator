...
**Workflow Recording with record_workflow**

The `record_workflow` tool captures user UI interactions and saves them as reusable YAML workflow files. This enables you to demonstrate a task once and then automate it repeatedly.

**Two-Step Recording Pattern:**

1. **Start Recording**: Call `record_workflow` with `action: \"start\"` to begin capturing UI events
2. **Stop Recording**: Call `record_workflow` with `action: \"stop\"` to end capture and save the workflow file

**Visual Highlighting During Recording:**

When recording workflows, you can enable real-time visual feedback that highlights UI elements as they're interacted with. This helps verify that the correct elements are being captured.

*   **Highlight Features:**
    - Colored borders around interacted elements (configurable BGR color)
    - Event type labels (\"CLICK\", \"TYPE\", \"SCROLL\", etc.) displayed on elements
    - Configurable duration for how long highlights persist
    - Low-energy mode option to reduce system load on slower machines

*   **BGR Color Format:** Colors use Blue-Green-Red format (opposite of RGB):
    - `0x0000FF` = Red (default)
    - `0xFF0000` = Blue
    - `0x00FF00` = Green
    - `0xFFFF00` = Cyan
    - `0xFF00FF` = Magenta
...
*   `action` (required): \"start\" to begin recording, \"stop\" to end and save
*   `workflow_name` (required when starting): Descriptive name for the workflow
*   `file_path` (optional): Custom save path (defaults to workflows directory)
*   `enable_highlighting` (optional, boolean): Enable visual feedback during recording
*   `highlight_color` (optional, u32): BGR color value for highlight borders
*   `highlight_duration_ms` (optional, u64): How long highlights persist (default: 500ms)
*   `highlight_show_labels` (optional, boolean): Show event type labels on highlights (default: true)
*   `low_energy_mode` (optional, boolean): Reduce system load for slower machines
*   `record_scroll_events` (optional, boolean): Include scroll events (default: false to reduce noise)

**Example: Recording with Highlighting Enabled**

```json
// Step 1: Start recording with red highlights lasting 1 second
{{
    ...
    \"args\": {{
        ...
        \"highlight_show_labels\": true
    }}
}}

// Step 2: Perform UI interactions (user demonstrates the task)
// - Clicks, typing, selections are automatically captured
// - Visual highlights appear in real-time showing what's being recorded

// Step 3: Stop recording and save workflow
{{
    \"tool_name\": \"record_workflow\",
    \"args\": {{
        \"action\": \"stop\"
...
**Example: Basic Recording Without Highlighting**
...
{{
    ...
    \"args\": {{
        \"action\": \"start\",
        \"workflow_name\": \"Data Entry Workflow\",
        \"file_path\": \"C:/workflows/data_entry.yml\"
    }}
}}

// Perform interactions, then stop
{{
    \"tool_name\": \"record_workflow\",
    \"args\": {{
        \"action\": \"stop\"
...
1. **Use Highlighting for Complex Workflows**: Enable highlighting when recording multi-step workflows to verify correct element capture
2. **Adjust Duration for Visibility**: Increase `highlight_duration_ms` to 1000-2000ms if interactions happen rapidly
3. **Choose Contrasting Colors**: Use colors that stand out against your application's UI (red/blue work well for most apps)
4. **Low-Energy Mode for Performance**: Enable `low_energy_mode` on slower machines to prevent recording lag
5. **Disable Scroll Events**: Keep `record_scroll_events: false` unless scrolling is essential to your workflow
6. **Descriptive Workflow Names**: Use clear, action-oriented names like \"Login Flow\" or \"Submit Report Process\"
...
- Element selection (checkboxes, radio buttons, dropdowns)
- Window activation and focus changes
- Scroll events (if enabled)

**Output Format:**

Workflows are saved as YAML files containing:
- Sequence of UI actions with precise selectors
- Timing information
- Element properties for reliable replay
- Comments describing each action
