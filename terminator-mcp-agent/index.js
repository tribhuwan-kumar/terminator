...
function getPlatformInfo() {
    ...
    if (platform === 'win32' && arch === 'x64') return { pkg: 'terminator-mcp-win32-x64-msvc', bin: 'terminator-mcp-agent.exe', npmDir: 'win32-x64-msvc' };
    if (platform === 'linux' && arch === 'x64') return { pkg: 'terminator-mcp-linux-x64-gnu', bin: 'terminator-mcp-agent', npmDir: 'linux-x64-gnu' };
    if (platform === 'darwin' && arch === 'x64') return { pkg: 'terminator-mcp-darwin-x64', bin: 'terminator-mcp-agent', npmDir: 'darwin-x64' };
    if (platform === 'darwin' && arch === 'arm64') return { pkg: 'terminator-mcp-darwin-arm64', bin: 'terminator-mcp-agent', npmDir: 'darwin-arm64' };
...
function addToClaudeConfig() {
    ...
    if (!fs.existsSync(configDir)) {
        console.error(`Claude desktop config directory does not exist: ${configDir}\nPlease make sure the Claude desktop app is installed for your platform.`);
        process.exit(1);
    }
    let config = {};
    if (fs.existsSync(configFile)) {
...
function addToVSCodeConfig() {
    ...
    try {
        ...
        const result = spawnSync(`${vscodeCmd} --add-mcp ${jsonArg}`, [], { stdio: 'inherit', shell: true });
        if (result.error) {
            if (result.error.code === 'ENOENT') {
                console.error("'code' command not found in PATH. Make sure VS Code CLI is installed and available.");
            } else {
...
function addToVSCodeInsidersConfig() {
    ...
    console.log('Adding Terminator MCP to VS Code Insiders via code-insiders CLI...');
    const mcpJson = {
        name: 'terminator-mcp-agent',
        command: 'npx',
        args: ['-y', 'terminator-mcp-agent']
    ...
    try {
        const { spawnSync } = require('child_process');
        const result = spawnSync(`${codeInsidersCmd} --add-mcp ${jsonArg}`, [], { stdio: 'inherit', shell: true });
        if (result.error) {
            if (result.error.code === 'ENOENT') {
                console.error("'code-insiders' command not found in PATH. Make sure VS Code Insiders CLI is installed and available.");
            } else {
                console.error('Failed to launch VS Code Insiders CLI:', result.error.message);
            }
            process.exit(1);
        }
        if (result.status !== 0) {
...
if (argv.includes('--add-to-app')) {
    ...
    const app = argv[appIndex] && !argv[appIndex].startsWith('--') ? argv[appIndex] : undefined;
    if (!app) {
        // Interactive prompt
        ...
        rl.question('Enter your choice (1-5): ', (answer) => {
            ...
            switch (answer.trim()) {
                case '1': selectedApp = 'cursor'; break;
                case '2': selectedApp = 'claude'; break;
                case '3': selectedApp = 'vscode'; break;
                case '4': selectedApp = 'insiders'; break;
                case '5': selectedApp = 'windsurf'; break;
                default:
...
if (argv.length === 0 || argv.includes('--start')) {
    ...
    function killProcess(proc) {
        ...
        if (process.platform === 'win32') {
            try { execSync(`taskkill /PID ${pid} /T /F`); } catch (e) { }
        } else {
            ...
            const termTimeout = setTimeout(() => {
                if (!child.killed) {
                    if (process.platform === 'win32') {
                        ...
                        setTimeout(() => { if (!child.killed) killProcess(child); }, 2000);
                    }
                }
            }, 2000);
